...
class VentaSala extends Component
{
    ...
    public function platosBuscados()
    {
        ...
        // Los platos siempre están disponibles (se preparan bajo demanda)
        return Plato::where(function($query) {
                $query->where('nombrePlato', 'LIKE', '%' . $this->busquedaPlatos . '%')
            ...
            ->map(function ($plato) {
                return [
                    ...
                    'stock' => 999, // Los platos siempre están disponibles
                    'tipo' => 'plato',
                ];
            })
            ->toArray();
    ...
    public function productosBuscados()
    {
        ...
        return Producto::where(function($query) {
                $query->where('nombre', 'LIKE', '%' . $this->busquedaProductos . '%')
    ...
    {
        return view('livewire.ventas.venta-sala');
    }
    ...
    public function agregarPlato($idPlato)
    {
        ...
        // Los platos se preparan bajo demanda, no necesitan validación de stock
        $key = "plato_$idPlato";
        
        if (isset($this->pedidoItems[$key])) {
            $this->pedidoItems[$key]['cantidad']++;
            $this->pedidoItems[$key]['subtotal'] = $this->pedidoItems[$key]['cantidad'] * $plato->precioVenta;
        } else {
            $this->pedidoItems[$key] = [
                ...
                'observacion' => '',
                'stockDisponible' => 999, // Sin límite para platos
            ];
        }
    ...
    public function agregarProducto($idProducto)
    {
        ...
        if (isset($this->pedidoItems[$key])) {
            ...
            $nuevaCantidad = $this->pedidoItems[$key]['cantidad'] + 1;
            if ($nuevaCantidad > $producto->stock) {
                session()->flash('error', "Stock insuficiente para '{$producto->nombre}'. Disponible: {$producto->stock}");
                return;
            }
            $this->pedidoItems[$key]['cantidad'] = $nuevaCantidad;
            $this->pedidoItems[$key]['subtotal'] = $nuevaCantidad * $producto->precioVenta;
        } else {
            $this->pedidoItems[$key] = [
                'tipo' => 'producto',
    ...
    public function actualizarCantidad($key, $cantidad)
    {
        ...
        if (isset($this->pedidoItems[$key])) {
            $item = $this->pedidoItems[$key];
            
            // Verificar stock disponible
            if (isset($item['stockDisponible']) && $cantidad > $item['stockDisponible']) {
                session()->flash('error', "Stock insuficiente para '{$item['nombre']}'. Disponible: {$item['stockDisponible']}");
                return;
            }
